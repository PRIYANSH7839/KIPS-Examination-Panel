...
if (isset($_SESSION['email'])) {
    if (@$_GET['q'] == 'addquiz' && $_SESSION['email'] == 'admin@kips.com') {
        $name    = $_POST['name'];
...
if (isset($_SESSION['email'])) {
    if (@$_GET['deidquiz'] && $_SESSION['email'] == 'admin@kips.com') {
        $eid = @$_GET['deidquiz'];
...
if (isset($_SESSION['email'])) {
    if (@$_GET['eeidquiz'] && $_SESSION['email'] == 'admin@kips.com') {
        $eid = @$_GET['eeidquiz'];
...
if (isset($_SESSION['email'])) {
    if (@$_GET['q'] == 'addqns' && $_SESSION['email'] == 'admin@kips.com') {
        $n   = @$_GET['n'];
        $eid = @$_GET['eid'];
        $ch  = @$_GET['ch'];
        for ($i = 1; $i <= $n; $i++) {
...
if (isset($_SESSION['email'])) {
    if (@$_GET['q'] == 'rmquiz' && $_SESSION['email'] == 'admin@kips.com') {
        $eid = @$_GET['eid'];
...
if (isset($_SESSION['email'])) {
    if (@$_GET['email'] && $_SESSION['email'] == 'admin@kips.com') {
        $email = @$_GET['email'];
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && isset($_POST['ans']) && (!isset($_GET['delanswer']))) {
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            $q = mysqli_query($con, "SELECT * FROM uanswers WHERE eid='$_GET[eid]' AND email='$_SESSION[email]' AND qid='$qid' ") or die('Error115');
            while ($row = mysqli_fetch_array($q)) {
                $prevans = $row['ans'];
            }
            ...
            $q = mysqli_query($con, "SELECT * FROM uanswers WHERE email='$_SESSION[email]' AND eid='$_GET[eid]' AND qid='$qid' ") or die('Error1977');
            if (mysqli_num_rows($q) != 0) {
                $q = mysqli_query($con, "UPDATE uanswers SET ans='$ans' WHERE email='$_SESSION[email]' AND eid='$_GET[eid]' AND qid='$qid' ") or die('Error197');
            } else {
                $q = mysqli_query($con, "INSERT INTO uanswers VALUES(NULL,'$qid','$ans','$ansid','$_GET[eid]','$_SESSION[email]')");
            }
            ...
            if ($ans == $ansid) {
                ...
                $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$eid' AND email='$_SESSION[email]' ") or die('Error115');
                while ($row = mysqli_fetch_array($q)) {
                    $s = $row['score'];
                    $r = $row['correct'];
                    $w = $row['wrong'];
                ...
                if (isset($prevans) && $prevans == $ansid) {
                    ...
                    $w--;
                    $s = $s + $correct + $wrong;
                    $q = mysqli_query($con, "UPDATE history SET score=$s,level=$sn,correct=$r,wrong=$w, date= NOW()  WHERE  email = '$_SESSION[email]' AND eid = '$eid'") or die('Error13');
                } else {
                    $r++;
                    $s = $s + $correct;
                    $q = mysqli_query($con, "UPDATE history SET score=$s,level=$sn,correct=$r, date= NOW()  WHERE  email = '$_SESSION[email]' AND eid = '$eid'") or die('Error14');
                }
            } else {
                ...
                $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$eid' AND email='$_SESSION[email]' ") or die('Error139');
                while ($row = mysqli_fetch_array($q)) {
                    $s = $row['score'];
                    $w = $row['wrong'];
                    $r = $row['correct'];
                ...
                if (isset($prevans) && $prevans != $ansid) {
                    ...
                    $w++;
                    $s = $s - $correct - $wrong;
                    $q = mysqli_query($con, "UPDATE history SET score=$s, level=$sn, wrong =$w, correct =$r, date= NOW()  WHERE  email = '$_SESSION[email]' AND eid = '$eid'") or die('Error11');
                } else {
                    $w++;
                    $s = $s - $wrong;
                    $q = mysqli_query($con, "UPDATE history SET score=$s,level=$sn,wrong=$w,date= NOW()  WHERE  email = '$_SESSION[email]' AND eid = '$eid'") or die('Error12');
                }
            }
            header("location:Userdash.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total") or die('Error152');
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && (!isset($_POST['ans'])) && (isset($_GET['delanswer'])) && $_GET['delanswer'] == "delanswer") {
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            ...
            $ans = $row['ans'];
            $q = mysqli_query($con, "DELETE FROM uanswers WHERE qid='$qid' AND email='$_SESSION[email]' AND eid='$_GET[eid]' ") or die("Error2222");
            if ($ans == $ansid) {
                $q = mysqli_query($con, "SELECT * FROM quiz WHERE eid='$eid' ") or die('Error129');
                while ($row = mysqli_fetch_array($q)) {
                    $wrong   = $row['wrong'];
                ...
                $r--;
                $s = $s - $correct;
                $q = mysqli_query($con, "UPDATE `history` SET `score`=$s,`level`=$sn,`correct`=$r, date= NOW()  WHERE  email = '$_SESSION[email]' AND eid = '$eid'") or die('Error11');
            } else {
                $q = mysqli_query($con, "SELECT * FROM quiz WHERE eid='$eid' ") or die('Error129');
                while ($row = mysqli_fetch_array($q)) {
                    $wrong   = $row['wrong'];
                ...
                $w--;
                $s = $s + $wrong;
                $q = mysqli_query($con, "UPDATE `history` SET `score`=$s,`level`=$sn,`wrong`=$w, date= NOW()  WHERE  email = '$_SESSION[email]' AND eid = '$eid'") or die('Error11');
            }
            header('location:Userdash.php?q=quiz&step=2&eid=' . $_GET[eid] . '&n=' . $_GET[n] . '&t=' . $total);
